...
const setupWaterAnimate = () => {
  let percent = 0
  let timeId = setInterval(function() {
    percent++
    if (percent >= props.totalPercentage) clearInterval(timeId)
    countRef.value.textContent = percent
    waterRef.value.style.transform = `translateY(${100 - percent}%)`
  }, 60)
}
</script>
